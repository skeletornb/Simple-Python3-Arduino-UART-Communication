...
class uart(object):
    ...
    def portquery(self):                            # Query connected devices's ports
        self.ports= list_ports.comports()
        for i in range(len(self.lConnected)):        # Emptying lConnected[]
...
class gui(object):
    def __init__(self, UART):
        ...
        self.puff = "0"
        self.root = Tk()
        self.root.title("Simple Python-Arduino UART")
    ...
    def portlistrefresh(self):
        ...
        if (len(self.connected)>5):
            self.mPorts.add_command(label=self.connected[5], command= lambda: self.portselect(5) )
        if (len(self.connected)>6):
            self.mPorts.add_command(label=self.connected[6], command= lambda: self.portselect(6) )
